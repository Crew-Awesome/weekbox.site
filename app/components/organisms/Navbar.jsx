...
export function Navbar() {
  ...
  const getLinkClasses = (path, isMobile = false) => {
    ...
    return `flex items-center ${isMobile ? 'justify-end w-full text-right' : 'justify-center'} px-5 py-2 rounded-full font-semibold transition-colors duration-300 !no-underline hover:!no-underline ${
      isActive 
    ...
    <>
      ...
      >
        ...
        <div className="hidden lg:flex gap-2 items-center">
          <a href="https://github.com/Crew-Awesome/Weekbox" target="_blank" rel="noreferrer" className="flex items-center justify-center w-10 h-10 rounded-full text-[var(--wb-on-surface-variant)] hover:bg-[var(--wb-item-hover)] hover:text-[var(--wb-on-surface)] transition-colors duration-300 !no-underline shadow-none" aria-label="GitHub">
          ...
          <a href="https://discord.gg/xQTtYF2Cfn" target="_blank" rel="noreferrer" className="flex items-center justify-center w-10 h-10 rounded-full text-[var(--wb-on-surface-variant)] hover:bg-[var(--wb-item-hover)] hover:text-[#5865F2] transition-colors duration-300 !no-underline shadow-none" aria-label="Discord">
          ...
          <div 
            ...
            onBlur={(event) => {
              if (!event.currentTarget.contains(event.relatedTarget)) setLangMenuOpen(false);
            }}
          >
            <button 
              type="button" 
              className={`px-4 py-2 rounded-full font-semibold bg-transparent border-0 cursor-pointer transition-colors duration-300 flex items-center gap-2 ${langMenuOpen ? 'bg-[var(--wb-item-hover)] text-[var(--wb-on-surface)]' : 'text-[var(--wb-on-surface-variant)] hover:bg-[var(--wb-item-hover)] hover:text-[var(--wb-on-surface)]'}`}
            ...
            {langMenuOpen && (
              <div id="language-menu" className="absolute top-[80%] right-0 mt-2 w-48 bg-[var(--wb-surface-container)] border border-[var(--wb-outline-variant)] rounded-2xl shadow-2xl flex flex-col py-2 z-50 overflow-hidden transform opacity-100 scale-100 transition-[opacity,transform] origin-top-right">
                {LANGUAGES.map((l) => (
                  <button 
                    key={l.code} 
        ...
        <div className="flex lg:hidden items-center">
          <button 
            ...
            className="flex items-center justify-center w-10 h-10 rounded-full text-[var(--wb-on-surface)] hover:bg-[var(--wb-item-hover)] transition-colors duration-300 bg-transparent border-0 cursor-pointer"
      ...
      >
        ...
        <button 
          type="button" 
          onClick={() => setIsMobileMenuOpen(false)}
          className="absolute top-4 right-4 flex items-center justify-center w-10 h-10 rounded-full text-[var(--wb-on-surface)] hover:bg-[var(--wb-item-hover)] transition-colors duration-300 bg-transparent border-0 cursor-pointer"
